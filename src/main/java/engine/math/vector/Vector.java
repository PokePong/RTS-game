...
public abstract class Vector<ResultType extends Vector<ResultType>> {
    ...
    public abstract float angleBetween(ResultType v);
     ...
     */
    public final ResultType sub(ResultType vec) {
     ...
     * returns the result. The values of this vector remain untouched.
     *
     * @param v the value to divide this vectors attributes by.
     * @return the result <code>Vector</code>.
     */
    ...
    public final ResultType sub(float s) {
        return add(-s);
    }

    public final ResultType mul(float scalar) {
        float[] a = toArray();
        for (int i = 0; i < a.length; ++i) {
            a[i] *= scalar;
        }
     ...
     * @return
     */
    public ResultType mul(ResultType v) {
        float[] a = toArray();
        float[] b = v.toArray();
        for (int i = 0; i < a.length; ++i) {
            a[i] *= b[i];
    ...
    public final float dot(ResultType v) {
        float[] a = toArray();
        float[] b = v.toArray();
        float result = 0;
        for (int i = 0; i < a.length; ++i) {
    ...
    public final ResultType interpolate(ResultType v, float changeAmnt) {
        float[] a = toArray();
        float[] b = v.toArray();
        for (int i = 0; i < a.length; ++i) {
    ...
    public final ResultType slerp(ResultType v, float changeAmnt) {
        ResultType res = interpolate(v, changeAmnt);
        float[] a = toArray();
        float[] b = res.toArray();
        for (int i = 0; i < a.length; ++i) {
            a[i] = a[i] + (b[i] - a[i]);
